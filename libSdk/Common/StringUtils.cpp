...
{
    std::string strOutPinyin;
    if (strChinese.empty())
        return "";

    const int length = strChinese.length();
    for (int j = 0, chrasc = 0; j < length;) 
    {
        // 非汉字处理
        if (strChinese.at(j) >= 0 && strChinese.at(j) < 128) 
        {
            strOutPinyin += strChinese.at(j);
            j++;
            continue;
        }
        // 汉字处理（GBK 双字节，须按无符号处理，否则 chrasc 为负导致查表失败）
        if (j + 1 >= length)
            break;  // 截断的尾字节，直接丢弃
        chrasc = static_cast<unsigned char>(strChinese.at(j)) * 256
                + static_cast<unsigned char>(strChinese.at(j + 1)) + 256;
